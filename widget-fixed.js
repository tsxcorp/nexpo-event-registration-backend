...
// Backend API Configuration with Auto-Detection
...
async function setupRealTimeFeatures(eventId) {
    try {
        ...
        if (healthCheck.status !== 'ok') {
            console.warn('⚠️ Backend health check failed, skipping real-time features');
            return;
        }
        
        // Check if Socket.IO is available and backend supports it
        const statusData = await makeApiRequest(BACKEND_CONFIG.ENDPOINTS.REALTIME_STATUS);
        
        if (statusData.success) {
            console.log('✅ Backend supports Socket.IO, connecting...');
            console.log('📊 Real-time status:', statusData.status);
            
            // Load Socket.IO library dynamically if not available
            ...
            console.log('⚠️ Real-time features not available, continuing without...');
        }
    } catch (error) {
...
async function loadTenantStats(userEmail) {
    try {
        console.log('📊 Loading tenant stats from Redis-enhanced backend...');
        
        // Use enhanced API request function
        const data = await makeApiRequestWithRetry(BACKEND_CONFIG.ENDPOINTS.EVENTS_LIST);
        
        if (data.success && data.events?.length > 0) {
            const event = data.events[0]; // Current event
            ...
            // Use Redis backend data for main stats
            ...
            if (groupQuantityElement) groupQuantityElement.textContent = event.group_registrations || 0; // Use same for now
            
            console.log('✅ Redis backend stats loaded:', {
            ...
            throw new Error('No events found in Redis backend data');
        }
    } catch (error) {
